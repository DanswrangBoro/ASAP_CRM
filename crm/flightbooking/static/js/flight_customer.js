...
const checkedBagsBySegment =
  flightOffer.travelerPricings[0].fareDetailsBySegment.reduce(
    (acc, segment) => {
      acc[
        segment.segmentId
...
numberTraveler.innerHTML = ` ${flightOffer.travelerPricings.length}`;
function generateForm() {
  if (currentTraveler === 0) {
    maxTravelers = flightOffer.travelerPricings.length;
    if (maxTravelers <= 0) {
  ...
  if (currentTraveler < maxTravelers) {
    ...
    travelerForm.innerHTML = `
            <p id="noT">${capitalizeFirstLetter(flightOffer.travelerPricings[currentTraveler - 1].travelerType)}
...
async function submitForm() {
  ...
  // Iterate over each person
  for (let i = 1; i <= flightOffer.travelerPricings.length; i++) {
...
function createCabinDetails(data) {
  ...
  cabinTypeDiv.textContent = `${flightOffer.travelerPricings[0].fareDetailsBySegment[0].cabin}`;
...
// console.log(flightOffer);
const segmentLength = flightOffer.itineraries.length;

function controlContainer() {
  for (let i = 0; i < segmentLength; i++) {
    createAndAppendStructures(flightOffer.itineraries[i]);
    createAndAppendStructuresBaggage(flightOffer.itineraries[i]);
  }
}
controlContainer();
